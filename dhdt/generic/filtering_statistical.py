...
def make_2D_Laplacian(alpha=.5):
    ...
    M = np.array([[diag_weight, cross_weight, diag_weight],
...
def normalized_sampling_histogram(Z):
    """ sample histogram based upon Sturges' rule, see also [StXX]_

    Parameters
    ----------
    Z : np.array, dim={1,n}, dtype={integer,float}
    ...
    References
    ----------
    .. [StXX] Sturges, "The choice of a class interval". Journal of the
       american statistical association. vol.21(153) pp.65–66.
    """
    sturge = 1.6 * (np.log2(Z.size) + 1)
    values, base = np.histogram(Z.flatten(), bins=int(np.ceil(sturge)))
