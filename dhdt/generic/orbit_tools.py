...
def nodal_rate_of_precession(i, a=None):
    ...
    omega = -3 / 2 * J2 * np.power(np.divide(a, p), 2) * n * np.cos(
        np.rad2deg(i))
    return omega
...
def orbital_calculation(ltime, radius, inclination, period, omega_0, lon_0):
    ltime, omega_0, lon_0 = np.squeeze(ltime), np.squeeze(omega_0), \
                            np.squeeze(lon_0)
    cta, gclat, gclon = _gc_calculation(ltime, period, inclination, omega_0,
                                        lon_0)
...
def observation_calculation(ltime, Sat, Gx, radius, inclination, period,
                            omega_0, lon_0):
    """
