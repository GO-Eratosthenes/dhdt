...
def list_central_wavelength_dc():
    ...
    d = {
         ...
         "common_name": pd.Series(common_name, dtype='str'),
...
def list_central_wavelength_sd(num_bands=4):
    ...
    acquisition_order = {"B1": 8, "B2": 1, "B3": 3, "B4": 4,
                         "B5": 2, "B6": 5, "B10":6, "B13":7}
    common_name = {"B1" : 'coastalblue', "B2" : 'blue', "B3" : 'green I',
                   "B4" : 'green II', "B5" : 'red', "B6" : 'yellow',
                   "B10" : 'rededge I', "B13" : 'nir', }
    d = {
         "center_wavelength": pd.Series(center_wavelength,
                                        dtype=np.dtype('float')),
         "full_width_half_max": pd.Series(full_width_half_max,
...
def planetscope_noradid():
    ...
    df = pd.read_csv(url_status,  escapechar='#',
                     delimiter="\t", delim_whitespace=False,
                     skiprows=5, error_bad_lines=False)
    df = df.rename(columns=lambda s: s.strip())
    # test: list(df.columns.values)
    df = df.rename(columns={'Planet_Name': 'platform',
                            'CATID': 'NORADID'})
    return df
