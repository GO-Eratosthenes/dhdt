...
def floating_algae_index(Red, Near, Swir, df):
    ...
    Red_prime = Red + np.multiply(
        (Swir - Red),
        np.divide(lambda_nir - lambda_red, lambda_swi - lambda_red))
    FAI = Near - Red_prime
    return FAI
...
def modified_simple_ratio(Red, Near):
    ...
    """
    MSR = np.divide(
        np.divide(Red, Near) - 1, np.sqrt(np.divide(Red, Near) + 1))
    return MSR
...
def atmospherically_resistant_vegetation_index(Blue,
                                               Red,
                                               Near,
                                               gamma=1.,
                                               L=.5):
    ...
    ARVI = np.divide(np.multiply(1 + L, Near - RB), Near + RB + L)
    return ARVI
