...
def make_shadowing(dem_path,
                   ...
                   nodata=-9999,
                   dtype=bool):
    (dem_mask, crs_dem, geoTransform_dem,
     targetprj_dem) = read_geo_image(os.path.join(dem_path, dem_file))
    if 'MSIL1C' in im_name:  # for Sentinel-2
        fname = os.path.join(im_path, im_name, '*B08.jp2')
        (crs_im, geoTransform_im, targetprj_im, rows, cols,
         bands) = read_geo_info(glob.glob(fname)[0])
    else:
        (crs_im, geoTransform_im, targetprj_im, rows, cols,
         bands) = read_geo_info(os.path.join(im_path, im_name))

    dem = make_same_size(dem_mask, geoTransform_dem, geoTransform_im, rows,
                         cols)
    msk = dem == nodata
    ...
    for swp in range(1, dem_rot.shape[0]):
        shw_rot[swp, :] = (dem_rot[swp, :]
                           < dem_rot[swp - 1, :] - dZ) & (cls_rot[swp, :] == 1)
        dem_rot[swp, :] = np.maximum(
            (dem_rot[swp, :] * cls_rot[swp, :]),
            (dem_rot[swp - 1, :] * cls_rot[swp - 1, :]) - dZ * cls_rot[swp, :])
    del dem_rot, cls_rot
    msk_new = ndimage.rotate(msk_rot,
                             -Az,
                             axes=(1, 0),
                             reshape=True,
                             output=None,
    ...
    if isinstance(dtype, (int, float)):
        ...
        shw_new = ndimage.rotate(shw_rot,
                                 -Az,
                                 axes=(1, 0),
                                 reshape=True,
                                 output=None,
...
def get_shadow_polygon(M, t_siz):
    ...
    super_pix = segmentation.slic(M, sigma=1, n_segments=mn,
...
def shadow_image_to_suntrace_list(M, geoTransform, az, method='nearest'):
    ...
    if method in (
        ...
        spl_order = 3
    # turn towards the sun
    M_rot = ndimage.rotate(M.astype(float),
                           180 - az,
                           axes=(1, 0),
                           reshape=True,
                           output=None,
    ...
    for k in range(M_rot.shape[1]):
        ...
        shade_exten = np.pad(shade_class, (1, 1),
                             'constant',
                             constant_values=0)
        ...
        suntrace_list = np.vstack(
            (suntrace_list,
             np.transpose(
                 np.vstack((x_beg[np.newaxis], y_beg[np.newaxis],
...
def label_occluder_and_casted(labeling, sunAz):
    ...
    for i in labList:
        ...
        subOrient = np.sign(bndOrient[labImin:labImax, labJmin:labJmax])
        ...
        for x in range(len(ridgeI)):  # loop through all occluders
            ...
            dI = -np.cos(
                np.radians(subAz[ridgeI[x]][ridgeJ[x]])
            )  # -cos # flip axis to get from world into image coords
            ...
            if len(castedI) > 1:
                ...
                shadowIdx[castedI[0] + labImin][castedJ[0] +
                                                labJmin] = -(x + 1)
...
def list_occluder_and_casted(labels, sunZn, sunAz, geoTransform):
    ...
    for shp, val in shapes(labels, mask=msk, connectivity=8):
        ...
        polyInnr = ndimage.binary_erosion(polyRast, np.ones((3, 3),
                                                            dtype=bool))
        ...
        for idx, _ in enumerate(ridgeI):  # ridgeI:
            castLine = find_polygon_intersect(ridgeI[idx], ridgeJ[idx],
                                              polygoon,
...
def find_polygon_intersect(ridge_i, ridge_j, polygoon, sun_az, sun_zn,
    ...
    castLine = np.array([ridge_x, ridge_y, cast_x, cast_y, sun_az, sun_zn])
    return castLine
