...
def masked_cosine_corr(I1, I2, M1, M2):  # todo
    ...
    if I1.ndim == 3:  # multi-spectral frequency stacking
        (mt, nt, bt) = I1.shape
        (ms, ns, bs) = I2.shape
        md, nd = np.round((ms - mt) / 2).astype(int), np.round(
            (ns - nt) / 2).astype(int)
...
def phase_only_corr(I1, I2):
    ...
    def _phase_only_corr_core(I1, I2):
        ...
        W2 = np.divide(1, np.abs(I2), out=np.zeros_like(I2), where=I2 != 0)
        Q = S1 * np.conj((W2 * S2))
        return Q
...
def normalized_gradient_corr(I1, I2):
    ...
    bands = I1.shape[2]
    for i in range(bands):  # loop through all bands
        Q_b = _normalized_gradient_corr_core(I1sub[..., i], I2sub[..., i], H_x,
                                             H_y)
        if i == 0:
            Q = Q_b.copy() / bands
        else:
...
def gaussian_transformed_phase_corr(I1, I2):
    ...
    bands = I1.shape[2]
    for i in range(bands):  # loop through all bands
        Q_b = _gaussian_transformed_phase_corr_core(I1sub[..., i], I2sub[...,
                                                                         i])
        if i == 0:
            Q = Q_b.copy() / bands
        else:
