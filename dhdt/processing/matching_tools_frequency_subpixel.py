...
def phase_gradient_descend(data,
                           W=np.array([]),
                           x_0=np.zeros((2)),
                           learning_rate=1,
                           n_iters=50):  # wip
...
def ransac(data,
    ...
    if initial_inliers is not None and len(initial_inliers) != num_samples:
        raise ValueError(
            "RANSAC received a vector of initial inliers (length %i)"
    ...
    spl_idxs = (initial_inliers if initial_inliers is not None else
                random_state.choice(num_samples, min_samples, replace=False))
    ...
    for num_trials in range(max_trials):
        ...
        for potential_param in np.arange(success):
            ...
            if cond_1 or cond_2:
                ...
                dynamic_max_trials = _dynamic_max_trials(
                    best_inlier_num, num_samples, min_samples,
                    stop_probability)
    ...
    def estimate(self, data, params_bound=0):
        ...
        if data.shape[0] >= 2:  # well determined
            if params_bound != 0:
                ...
                x_stack = np.linalg.lstsq(data[:, 0:2],
                                          data[:, -1],
                                          rcond=None)[0]
        else:  # under-determined
            raise ValueError('At least two vectors needed.')
...
def phase_ransac(data, max_displacement=1, precision_threshold=.05):
    ...
    if data.shape[0] == data.shape[1]:
        ...
        data = np.vstack((Fy.flatten(), Fx.flatten(), Q.flatten())).T
