...
def maximum_likelihood(I1, I2):
    ...
    C = ndimage.generic_filter(I2,
                               maximum_likelihood_func,
                               size=I1.shape,
                               extra_keywords={'I1': I1})
    C[C > 1.4] = 0
    return C
...
def weighted_normalized_cross_correlation(I1, I2, W1=None, W2=None):
    ...
    if np.all(W1 == 1) and np.all(W1 == 1):
        return normalized_cross_corr(I1, I2)
    ...
    wght_var_2 = (view_I2 - np.tile(wght_mu2[:, :, np.newaxis, np.newaxis],
                                    (1, 1, m1, n1)))
    del view_I2
    sum_view_W2 = np.sum(np.sum(view_W2, axis=-2), axis=-1)
    sum_view_W2_stack = np.tile(sum_view_W2[:, :, np.newaxis, np.newaxis],
                                (1, 1, m1, n1))
    wght_var_W2 = np.divide(np.multiply(view_W2, wght_var_2),
                            sum_view_W2_stack,
                            where=sum_view_W2_stack != 0,
                            out=np.zeros_like(sum_view_W2_stack))
    del sum_view_W2_stack, view_W2
    wght_var_W1_stack = np.repeat(np.repeat(wght_var_W1[np.newaxis,
                                                        np.newaxis, :],
                                            m2,
                                            axis=0),
                                  n2,
    ...
    # put terms together
    wght_cov_12 = np.sum(np.sum(np.multiply(wght_var_W1_stack, wght_var_W2),
                                axis=-2),
                         axis=-1)
    wght_cov_12 = np.divide(wght_cov_12,
                            elem_M12,
                            where=elem_M12 != 0,
...
def cosine_similarity(I1, I2):
    ...
    Going along via different names, such as Gradient Inner Product
    (GIP)[Gl08]_ or Cosine Similarity [De21]_.
    ...
    References
    ----------
    .. [Gl08] Glocker et al. "Optical flow estimation with uncertainties
              through dynamic MRFs" IEEE conference on computer vision and
              pattern recognition, 2008.
    .. [De21] Dematteis et al., "Comparison of digital image correlation
              methods and the impact of noise in geoscience applications"
              Remote sensing, vol.13(2), pp.327, 2021.
    """
