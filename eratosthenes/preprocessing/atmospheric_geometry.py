...
def get_height_tropopause(lat):
    ...
    Parameters
    ----------
    lat : {float, np.array}, unit=degrees, range=-90...+90
        latitude of interest

    Returns
    -------
    h_t : {float, np.array}, unit=meters
...
def get_T_sealevel(lat, method='noerdlinger'):
    ...
    Returns
    -------
    T_sl : {float, np.array}, unit=Kelvin
...
def get_water_vapor_press(T):
    ...
    Parameters
    ----------
    t : {float, np.array}, unit=Kelvin
        temperature
    Returns
    -------
    P_w : {float, np.array}, unit=Pascal
...
def refractive_index_broadband_vapour(sigma):
    ...
    Parameters
    ----------
    sigma : {float, np.array}, unit=µm-1
...
def ciddor_eq5(rho_a, rho_w, n_axs, n_ws, rho_axs, rho_ws):
    ...
    Parameters
    ----------
    rho_a : {np.array, float}, unit=kg m-3
        density of dry air
    rho_w : (np.array, float}, unit=kg m-3
    n_axs : np.array, size=(k,)
        refractive index for dry air at sea level at a specific wavelength
    n_ws : np.array, size=(k,)
...
def refraction_angle_analytical(zn_0, n_0):
    ...
    Parameters
    ----------
    zn_0 : {float, np.array}, unit=degrees
    ...
    Returns
    -------
    zn : {float, np.array}, unit=degrees
