...
def test_match_pair(im_size=2 ** 7, temp_size=2 ** 4, tolerance=10):
    ...
    for bands in [1, 3]:  # testing both grayscale and multi-spectral data
        ...
        _test_frequency_correlators(im1,
                                    im2,
                                    geoTransform,
                                    X_samp,
                                    Y_samp,
...
def _test_spatial_correlators(im1,
                              im2,
                              geoTransform,
                              X_samp,
                              Y_samp,
    ...
    for correlator in corr_list:
        ...
        X_disp, Y_disp, _ = match_pair(im1,
                                       im2,
                                       msk1,
                                       msk2,
                                       geoTransform,
...
def _test_frequency_correlators(im1,
                                im2,
                                geoTransform,
                                X_samp,
                                Y_samp,
...
def test_photohypsometric_coupling(N=10, Z_shape=(400, 600), tolerance=20):
    ...
    with tempfile.TemporaryDirectory(dir=os.getcwd()) as tmpdir:
        ...
        dh = read_conn_files_to_stack(None,
                                      conn_file=conn_list,
...
def test_photohypsometric_refinement_by_same(
        Z_shape=(400, 600), tolerance=1.0, weight=False):
    Z, geoTransform = create_artificial_terrain(Z_shape[0], Z_shape[1])
    ...
    pix_dispersion = np.nanmedian(
        np.hypot(post_1[:, 0] - post_2_new[:, 0],
...
def _test_photohypsometric_refinement(N, Z_shape, tolerance=0.1):
    ...
    with tempfile.TemporaryDirectory(dir=os.getcwd()) as tmpdir:
        ...
        for idx in range(match_idxs.shape[1]):
            ...
            ax1.imshow(Isub_1)
            ax1.scatter(np.mod(j_1[cnt], 1) + w,
                        np.mod(i_1[cnt], 1) + h,
                        marker='+')
            ax2.imshow(Isub_2)
            ax2.scatter(np.mod(j_2[cnt], 1) + w,
                        np.mod(i_2[cnt], 1) + h,
                        marker='+')
